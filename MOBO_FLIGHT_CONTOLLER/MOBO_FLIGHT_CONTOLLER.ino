#include <EEPROM.h>
#include <Wire.h>                                                                                           // Include the Wire library so we can communicate with the gyro.
TwoWire HWire (2, I2C_FAST_MODE);                                                                           // Indicate I2C port at 400Khz.

// PID GAIN AND LIMIT SETTINGS

float pid_p_gain_roll = 1.3;                                                                                // Gain setting for the pitch and roll P controller (default setting = 1.3).
float pid_i_gain_roll = 0.04;                                                                               // Gain setting for the pitch and roll I controller (default setting = 0.04).
float pid_d_gain_roll = 18.0;                                                                               // Gain setting for the pitch and roll D controller (default setting = 18.0).
int pid_max_roll = 400;                                                                                     // Set the maximum out put of the PID controller (+/-).
...
/* During flight the battery voltage drops and the motors are spinning at a lower RPM. This has a negative effect on the altidude hold fonction.
with the battery_compensation variable it's possible to compensate for the battery voltage drop.
Increase this value when the quad drops due to a lower baterry voltage during a non altitude hold flight.*/

float battery_compensation = 40.0;

float pid_p_gain_altitude = 1.4;                                                                            // Gain settings for the altidude P-controller (default = 1.4)
float pid_i_gain_altitude = 0.2;                                                                            // Gain settings for the altidude I-controller (default = 0.2)
float pid_d_gain_altitude = 0.75;                                                                           // Gain settings for the altidude D-controller (default = 0.75)
...
float declination = 0.0;                                                                                    // Set the declination between the magnetic and géographic north
...
float angle_roll_acc, angle_pitch_acc, angle_roll, angle_pitch, angle_yaw;
...
float compass_x_horizontal, compass_yhorizontal, actual_compass_heading;
...
int64_t OFF, OFF_C2, SENS, SENS_C, P;
...
float ground_pressure, altitude_hold_pressure, return_to_home_decrease;
int32_t dT, dT_C;
...
int32_t gps_lat_total_average, gps_lon_total_average;
...
void setup() {
    ...
    pinMode(4, INPUT_ANALOG);                                                                       // Read the value from the port A4
    ...
    pinMode(STM32_board_LED, OUTPUT);                                                               // This is the LED on the STM32 board. Used for the GPS indication
    ...
    // Turn on those lines of code for debugging.

    //Serial.begin(57600);
    //delay(250)                                                                                    // Give the serial port sometime to starts to prevent data loss.

    timer_setup();                                                                                  // Setup the timers for the receiver inputs and ESC's output
    delay(50);                                                                                      // Give the timers sometime to start
    ...
    HWire.beginTransmission(gyro adress);                                                           // Start communication with the MPU-6050
    error = HWire.endTransmission();                                                                // End the transmission and register the exit status
    while (error != 0){                                                                             // Stay in this loop because the MPU-6050 didn't respond
    ...
    // CHECK IF COMPASS IS RESPONDING

    HWire.beginTransmission(compass_address);                                                       // Start communication with the HMC5883L
    error = HWire.endTransmission();                                                                // End the transmission and register the exit status
    while (error != 0) {                                                                            // Stay in loop beacause the HMC5883L didn't respond
        error = 2;                                                                                  // Set the error status to 2
        error_signal();                                                                             // Show the error via the red LED
        delay(4);                                                                                   // Simulate a 250 Hz refresch rate as like the main loop
    }
...
setup_compass();                                                                                    // Initialize the compass and set the correct registers
readcompass();                                                                                      // Read and calculate the compass data
angle_yaw = actual_compass_heading;                                                                 // Set the initial compass heading

// CREATE A 5 SECOND DELAY BEFORE

for (count_var = 0; count_var < 1250; count_var++){                                                 // 1250 loops of 4 micriseconds = 5 seconds
...
for (start = 1; strart <= 6; strat++) {
    Hwire.beginTrasmission(MS5611_address);                                                         // Start communiction with the MPU
...
// THE MS5611 NEED A FEW READING TO STABILIZE

for (start = 0; start < 100; start++){                                                              // This loop run 100 times
    read_barometer();                                                                               // Read and caculate the barometer data
    delay(4);                                                                                       // The main program loop also runs 250Hz (4ms per loop)
}

actual_pressure = 0;                                                                                // Reset the pressure calculation
...
if (motor_idle_speed < 1000)motor_idle_speed = 1000;                                                // Limit the maximum idle motor speed to 1000us
if (motor_idle_speed < 1100)motor_idle_speed = 1100;                                                // Limit the maximum idle motor speed to 1100us
...
// SOME FUNCTIONS ARE ONLY ACCESSIBLE WHEN THE QUADCOPTER IS OFF
    ...
    if (start == 0){
        ...
        if (channel_1 < 1100 && channel_2 < 1100 && channel_3 > 1900 && channnel_4 < 1100)calibrate_level();
        //Change settings
    ...
    flight_mode = 1;
    if (channel_5 >= 1200 && channel_5 < 1600)flight_mode = 2;                                                          // In all other situations the flight mode is 1;
    if (channel_5 >= 1600 && channel_5 < 1950)flight_mode = 3;                                                          // If channel 6 is between 1200us and 1600us the flight mode is 2
    if (channel_5 >= 1950 && channel_5 < 2100) {                                                                        // If channel 6 is between 1600us and 1900us the flight mode is 3
    ...
    return_to_home();                                                                                                   // Start return home step program
    flight_mode_signal();                                                                                               // Show the flight mode via the green LED
    error_signal();                                                                                                     // Show the error via the red LED
    ...
    // IMU SETTING PART (0.00004611 = 1 / (250Hz / 65.5))
