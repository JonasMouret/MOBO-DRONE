...
void gps_setup(void) {
    ...
    delay(300);                                                                                                     // A small delay is added to give the GPS some time to respond (9600bps)
    ...
    // Set baud rate to 57.6Kbps by using the ublox protocol
...
void read_gps(void) {
    ...
    while (Serial1.available() && new_line_found == 0) {
        char read_serial_byte = Serial1.read();
        if (read_serial_byte == '$'){
    ...
    if (new_line_found = 1){
        ...
        if (incomming_message[4] == 'L' && incomming_message[5] == 'L' && incomming_message[7] == ',') {
            digitalWrite(STM32_board_LED, !ditalRead(STM32_board_LED));

            // Set some variables to 0 if no valid information is find by the GPS module. This is needed for GPS lost when flying
        ...
        // If the line start with GA and if there is a GPS fix we can scan the line for the Latitude, Longitude and number of satelites.
        ...
        if(incomming_message[4] == 'G' && incomming_message[5] = 'A' && incomming_message[44] == '1' || incomming_message[44] == '2') {
            ...
            if (incomming_message[42] == 'E') longitude_east = 1;                                                                           // When flying east of the prime meridian the longitude_east will be set to 1
            else longitude_east = 0;                                                                                                        // When flying east of the prime meridian the longitude_east will be set to 0
        
            number_used_sats = ((int)incomming_message[46] - 48) * (long) 10;                                                               // Filter the number of satelites from GGA line
            number_used_sats += (int)incomming_message[47] - 48;                                                                            // Filter the number of satelites fromm GGA line

            if (lat_gps_previous == 0 && lat_gps_previous == 0) {                                                                           // If this is the first time the GPS is used
                lat_gps_previous =  lat_gps_actual;                                                                                         // Set the lat_gps_previous variable to lat_gps_actual varialble
                lon_gps_previous = lon_gps_actual;                                                                                          // Set the lon_gps_previous variable to lon_gps_actual varialble
            }
            ...
            // The GPS is set to a 5Hz refresh rate. Between every 2 GPS measurments,ç GPS values are simulated

            gps_add_counter = 5;                                                                                                            // Set the gps_add_counter variable to 5 as countdown loop timer
            new_gps_data_counter = 9;                                                                                                       // Set the new_gps_data_counter variable to 9. This is the number of simulated values between 2 GPS measurments.
            lat_gps_add = 0;
            lon_gps_add = 0;
            new_gps_data_available =1;                                                                                                      // Set the new_gps_data_available to indicate that there is a new data available
        }

        // If the line start with SA and if there is a GPS fix we can scan the line for the fix type (none, 2D or 3D)
    ...
    if(gps_add_counter == 0 && new_gps_data_counter > 0) {                                                                                  // If gps_add_counter is 0 and there are new GPS simulations needed
        ...
        new_gps_data_counter --;                                                                                                            // Decrement the new_gps_data_counter to get only 9 simulations
        gps_add_counter = 5;                                                                                                                // Set the gps_add_counter variable to 5 as a count down loop timer

        lat_gps_add += lat-gps_loop_add;                                                                                                    // Add the simulated part to a buffer float variable because the l_lat_gps can only hold intergers
...
if (new_gps_data_available) {                                                                                                               // If there is a new set of GPS data available
    ...
    else digitalWrite(STM32_board_LED, LOW);                                                                                                // Turn the LED on the STM solid on ( LED function inverted)
...
if (flight_mode >= 3 && waypoint_set ==1 ) {
    ...
    // GPS STICK MOVE ADJUSMENT
    if(flight_mode == 3 && takeoff_detected == 1) {
        
        // South Correction
        
        if(!latitude_north) {
            l_lat_gps_float_adjust += 0.0015 * (((channel_2 - 1500) * cos(gps_man_adjust_heading * 0.017453)) + ((channel_1 - 1500) * cos((gps_man_adjust_heading - 90) * 0.017453)));
        }
