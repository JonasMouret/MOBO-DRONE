                                                    ///////////////////////////////////////////
                                                    /// CALIBRATION PROGRAM COMPASS / LEVEL ///
                                                    ///////////////////////////////////////////

void calibrate_compass(void) {

    compass_calibration_on = 1;                                                                                             // Set the compass_calibration_on to disable the adjusment of the raw compass values
    red_led(HIGH);                                                                                                          // The red led will indicate that the compass calibration is active
    green_led(LOW);                                                                                                         // Turn of the green LED

    while (channel_2 < 1900) {                                                                                              // Stay in this position until the pilot lowers the pitch stick of the transmitter
        send_telemetry_data();                                                                                              // Send telemetry data to the ground station
        delayMicroseconds(3700);                                                                                            // Simulate a 250Hz program loop
        read_compass();                                                                                                     // Read the raw compass value

        // In this code the Max and Min values of the compass are detected and stored.
    ...
    setup_compass();                                                                                                        // Initiallize the compass and set the correct register
    read_compass();                                                                                                         // Read and calculate the compass data 
    angle_yaw = actual_compass_heading;                                                                                     // Set the initialize compass heading
    ...
    loop_timer ) micros();                                                                                                  // Set the timer for the next loop_timer
}
